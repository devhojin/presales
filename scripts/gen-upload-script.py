# -*- coding: utf-8 -*-
"""
upload-product-files.mjs를 생성하는 Python 헬퍼 스크립트
Supabase Storage가 한글/특수문자 파일명을 거부하므로
Storage 경로는 ASCII 안전한 file_NNN.ext 형식으로 저장
"""
...
js_rows = []
for pid, folder, fname in mappings:
    # JSON.stringify 방식으로 이스케이프
    folder_js = folder.replace("\\", "\\\\").replace('"', '\\"')
    fname_js = fname.replace("\\", "\\\\").replace('"', '\\"')
    js_rows.append('  [%d, "%s", "%s"]' % (pid, folder_js, fname_js))

maps_block = "const MAPS = [\n" + ",\n".join(js_rows) + "\n];\n"

script = r"""// upload-product-files.mjs
...
out_path = "C:/Users/hojin/presales/scripts/upload-product-files.mjs"
