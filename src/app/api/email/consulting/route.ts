...
export async function POST(request: NextRequest) {
  try {
    ...
    const applicantBody = `
      ...
      <p style="margin:0 0 32px;font-size:14px;color:#64748b;">${req.name}님, 문의해 주셔서 감사합니다. 24시간 이내에 담당자가 연락드리겠습니다.</p>
      ...
      <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:20px;margin-bottom:24px;">
        <table width="100%" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <td style="font-size:13px;color:#334155;padding-bottom:10px;font-weight:500;">${req.name}</td>
          </tr>
          ${req.company ? `
          <tr>
            <td style="font-size:13px;color:#64748b;padding-bottom:10px;vertical-align:top;">회사명</td>
            <td style="font-size:13px;color:#334155;padding-bottom:10px;">${req.company}</td>
          </tr>` : ''}
          ${req.phone ? `
          <tr>
            <td style="font-size:13px;color:#64748b;padding-bottom:10px;vertical-align:top;">연락처</td>
            <td style="font-size:13px;color:#334155;padding-bottom:10px;">${req.phone}</td>
          </tr>` : ''}
          <tr>
            <td style="font-size:13px;color:#64748b;padding-bottom:10px;vertical-align:top;">요금 방식</td>
            <td style="font-size:13px;color:#334155;padding-bottom:10px;font-weight:500;">${packageLabel}</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td style="font-size:13px;color:#334155;line-height:1.6;">${req.message.replace(/\n/g, '<br/>')}</td>
          </tr>` : ''}
        </table>
      </div>
    ...
    const adminBody = `
      ...
      <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:20px;margin-bottom:24px;">
        <table width="100%" cellpadding="0" cellspacing="0">
          <tr>
            <td style="font-size:13px;color:#64748b;padding-bottom:10px;vertical-align:top;width:100px;">신청자</td>
            <td style="font-size:13px;color:#334155;padding-bottom:10px;font-weight:600;">${req.name}</td>
          </tr>
          <tr>
            ...
            <td style="font-size:13px;padding-bottom:10px;">
              <a href="mailto:${req.email}" style="color:#1e40af;text-decoration:none;">${req.email}</a>
            </td>
          </tr>
          ${req.phone ? `
          <tr>
    ...
    await sendEmail(
      ADMIN_EMAIL,
      `[프리세일즈 관리자] 새 컨설팅 문의 - ${req.name} (${packageLabel.split(' ')[0]})`,
