...
export default function NewProduct() {
  ...
  const inputClass = "w-full px-3 py-2 border border-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
  ...
  return (
    <div className="max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-border p-6 space-y-5">
          <h2 className="font-semibold text-foreground">상세 정보</h2>
        ...
        <div className="bg-white rounded-xl border border-border p-6 space-y-4">
          <h2 className="font-semibold text-foreground">공개 설정</h2>
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" className="w-4 h-4 rounded border-border" checked={form.is_published}
              onChange={(e) => setForm({ ...form, is_published: e.target.checked })} />
            <span className="text-sm">바로 공개</span>
          </label>
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" className="w-4 h-4 rounded border-border" checked={form.is_free}
        ...
        <div className="flex gap-3">
          <button
            type="submit"
            disabled={saving}
            className="h-10 px-6 rounded-xl bg-primary text-white text-sm font-medium hover:bg-primary/90 disabled:opacity-50 transition-colors"
          >
            {saving ? '등록 중...' : '상품 등록'}
          </button>
          <Link
            href="/admin/products"
            className="h-10 px-6 rounded-xl border border-border text-sm font-medium hover:bg-muted inline-flex items-center transition-colors"
          >
            취소
          </Link>
        </div>
