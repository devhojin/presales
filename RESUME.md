...
- 결제 완료 후 구매자에게 카카오톡 알림을 보내는 기능입니다.
- 구현 전 카카오 알림 방식부터 확정할 것: 알림톡, 채널 메시지, 또는 다른 공식 메시지 API.
- 성공 기준:
  - 결제 완료 시점과 무통장/가상계좌 입금 완료 웹훅 시점 모두에서 중복 없이 발송
  - 주문번호, 상품명, 결제금액, 마이페이지 다운로드 링크 포함
  - 발송 성공/실패 로그 저장
  - 실패 시 관리자 확인 가능
  - 사용자가 이메일만 있고 휴대폰 번호가 없을 때의 대체 흐름 정의
